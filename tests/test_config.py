...
def test_save_and_load(tmp_path, monkeypatch):
    ...
    cfg = Config(openai_api_key="sk-test", soniox_api_key="so-test", ui_lang="ja")
    save_config(cfg)
...
def test_load_migrates_legacy_api_key(tmp_path, monkeypatch):
    ...
    config_file.write_text(
        json.dumps({"api_key": "sk-x", "ui_lang": "ko"}),
        encoding="utf-8",
    )
...
def test_load_migrates_target_lang_to_ui_lang(tmp_path, monkeypatch):
    """구 config의 target_lang 키가 ui_lang으로 마이그레이션된다."""
    config_file = tmp_path / "config.json"
    config_file.write_text(
        json.dumps({"openai_api_key": "sk-x", "target_lang": "ja"}),
        encoding="utf-8",
    )
    monkeypatch.setattr("vrt.config.CONFIG_FILE", config_file)

    cfg = load_config()
    assert cfg.ui_lang == "ja"
    assert cfg.openai_api_key == "sk-x"


def test_load_target_lang_ignored_when_ui_lang_present(tmp_path, monkeypatch):
    """ui_lang과 target_lang이 모두 있으면 ui_lang 값을 사용하고 target_lang은 무시된다."""
    config_file = tmp_path / "config.json"
    config_file.write_text(
        json.dumps({"openai_api_key": "sk-x", "ui_lang": "en", "target_lang": "ja"}),
        encoding="utf-8",
    )
    monkeypatch.setattr("vrt.config.CONFIG_FILE", config_file)
...
def test_load_ignores_unknown_fields(tmp_path, monkeypatch):
    ...
    config_file = tmp_path / "config.json"
    config_file.write_text(
        json.dumps({"openai_api_key": "sk-x", "ui_lang": "ko", "unknown_field": "ignored"}),
        encoding="utf-8",
    )
    monkeypatch.setattr("vrt.config.CONFIG_FILE", config_file)
